...
describe FetchEspnApiData::Teams do
  describe ".perform" do
    context "given a valid league_id" do
      it "returns a json response of teams with 13 teams" do
        response = FetchEspnApiData::Teams.perform('1310767', '2018')
      ...
      it "returns a team with the correct attributes" do
        response = FetchEspnApiData::Teams.perform('1310767', '2015')
        team = response[0]
        expect(team["firstName"]).to eq("michael")
        expect(team["lastName"]).to eq("rode")
        expect(team["isLeagueCreator"]).to eq(true)
    ...
    context "when passed an invald league id" do
      it "returns the proper error message" do
        expect{response = FetchEspnApiData::Teams.perform('999999999', '2018')}.to raise_error(ApiExceptions::RequestError::InvalidLeagueError, 'This is not a valid league_id.')
      end
    end
    
    context "when passed an invalid year" do
      it "returns the proper error message" do
        expect{response = FetchEspnApiData::Teams.perform('1310767', '2019')}.to raise_error(ApiExceptions::RequestError::InvalidYearError, 'This is not a valid year for this league.')
      end
    end
  end
end